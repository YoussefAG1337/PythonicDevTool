...
def backup_command(
    folder_path: Path = typer.Argument(
        ..., help="Directory to backup", exists=True, file_okay=False, resolve_path=True
    ),
    dest_path: Path = typer.Option(
        Path.home()
        / "devtool_backups",  # Defaults to a folder in the user's home directory
        "--dest",
        "-d",
    ...
    try:
        ...
        with console.status(
            f"[cyan]Compressing {folder_path.name}...[/cyan]", spinner="dots"
        ):
